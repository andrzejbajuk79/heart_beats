	...
	var sex =document.getElementById('sex').value;
		...
		function male() {
			if(diff_years<=3){
			    ...
			    heart_bets_total = age_child*130+(age_kid )*100 +(age_teen)*85 + (diff - age_teen_total)*70;
			}else{
		...
		function women() {
			if(diff_years<=3){
				// calc_beats_child(130);
				heart_bets_total= diff*135;
			}else if(diff_years<=8){
				heart_bets_total = age_child*135+(diff-age_child)*105;
			}else if(diff_years<=18){
				heart_bets_total = age_child*135+(age_kid )*105 + (diff- age_kid_total)*90;
			}else if(diff_years<=60){
			    heart_bets_total = age_child*135+(age_kid )*105 +(age_teen)*90 + (diff - age_teen_total)*80;
			}else{
				heart_bets_total = age_child*135 + (age_kid )*105 + (age_teen)*90 + (age_matur)*75 +(diff-age_matur_total)*60;			
			}
		}
