...
$(document).ready(function() {
	...
	var diff2=now.diff(birthday,'minutes')  ;
	if(!minutes){
		minutes =0;
	}

	var diff = diff2- minutes; //czas w minutach od urodzin po odjeciu godziny urodzenia
debugger;
	if(birthday){
		if(diff>0){
		...
		if(diff_years<=3){
		    ...
		    heart_bets_per_min = age_child*130+(age_teen)*100 + (diff - age_teen_total)*70;
		}else{
			heart_bets_per_min = age_child*130 + (age_teen)*100 + (age_matur)*70 +(diff-age_matur_total)*60;			
		}
